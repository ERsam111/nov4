...
export const NetworkMap = ({ customerData, facilityData, productData, transportationData }: NetworkMapProps) => {
  ...
  const filteredConnections = selectedProduct === "All" 
    ? transportationData 
    : transportationData.filter((t: any) => t["Product Name"] === selectedProduct);
  ...
  return (
    <Card className="border-border shadow-lg">
      ...
      <CardContent>
        <div className="relative bg-accent/10 rounded-lg border border-border" style={{ height: '600px' }}>
          {/* SVG for connections */}
          <svg className="absolute inset-0 w-full h-full pointer-events-none">
            {filteredConnections.map((conn: any, idx: number) => {
              ...
              const destination = customerData.find((c: any) => c["Customer Name"] === conn["Destination Name"]) 
                || facilityData.find((f: any) => f["Facility Name"] === conn["Destination Name"]);
              ...
              const originIndex = originIsSupplier 
                ? suppliers.findIndex((s: any) => s["Facility Name"] === origin["Facility Name"])
                : dcs.findIndex((d: any) => d["Facility Name"] === origin["Facility Name"]);
              
              const originPos = originIsSupplier 
                ? getNodePosition(originIndex, suppliers.length, 'left')
                : getNodePosition(originIndex, dcs.length, 'center');
              
              const destIndex = destIsCustomer 
                ? customerData.findIndex((c: any) => c["Customer Name"] === destination["Customer Name"])
                : dcs.findIndex((d: any) => d["Facility Name"] === destination["Facility Name"]);
              
              const destPos = destIsCustomer 
                ? getNodePosition(destIndex, customerData.length, 'right')
                : getNodePosition(destIndex, dcs.length, 'center');
              
              return (
                <line
                  key={`${conn["Origin Name"]}-${conn["Destination Name"]}-${idx}`}
                  x1={originPos.x}
                  y1={originPos.y}
                  x2={destPos.x}
                  y2={destPos.y}
