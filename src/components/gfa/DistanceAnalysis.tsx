...
export function DistanceAnalysis({ dcs, customers, rangeStep, onRangeStepChange, distanceUnit }: DistanceAnalysisProps) {
  const [isOpen, setIsOpen] = useState(true); // Default open
  const [selectedSites, setSelectedSites] = useState<Set<string>>(
    new Set(dcs.map(dc => dc.id))
  );
  ...
  const distanceAnalysis = useMemo(() => {
    ...
    if (filteredDcs.length === 0) {
      return { ranges: [], totalDemand: 0, chartData: [] };
    }
    ...
    const customerDistances: { customer: Customer; distance: number }[] = [];
    ...
    filteredDcs.forEach(dc => {
      dc.assignedCustomers.forEach(customer => {
        ...
        customerDistances.push({ customer, distance });
      });
    });
    ...
    for (let i = 0; i < numRanges; i++) {
      ...
      const rangeCustomers = customerDistances.filter(
        cd => cd.distance >= minKm && cd.distance < maxKm
      );
      
      const rangeDemand = rangeCustomers.reduce(
        (sum, cd) => sum + (cd.customer.demand * cd.customer.conversionFactor),
        0
      );
    ...
    return { ranges, totalDemand, chartData };
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-1 text-left">
                ...
                <p className="text-sm text-muted-foreground">
                  Demand fulfillment by distance ranges
                </p>
              </div>
            </div>
          </CollapsibleTrigger>
            ...
            {dcs.map(dc => (
              <div key={dc.id} className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={() => toggleSite(dc.id)}
                />
                <Label
                  htmlFor={`site-${dc.id}`}
            ...
            <div className="space-y-2">
              ...
              <ResponsiveContainer width="100%" height={300}>
                {chartView === "cumulative" ? (
                  <LineChart data={distanceAnalysis.chartData}>
                    ...
                    <YAxis 
                      className="text-xs"
                      tick={{ fill: 'hsl(var(--muted-foreground))' }}
                      label={{ value: '% of Demand', angle: -90, position: 'insideLeft' }}
                    />
                    <Tooltip 
                      contentStyle={{ 
                  ...
                  <ComposedChart data={distanceAnalysis.chartData}>
                    <CartesianGrid strokeDasharray="3 3" className="stroke-muted" />
                    <XAxis 
                      dataKey="range" 
                      className="text-xs"
                    ...
                    <Bar 
                      ...
                      name="Range %"
                    >
                      {distanceAnalysis.chartData.map((entry, index) => (
                        <Cell 
                          key={`cell-${index}`} 
            ...
            <div className="space-y-2">
              ...
              <div className="border rounded-lg overflow-hidden">
                <Table>
                  ...
                  <TableBody>
                    {distanceAnalysis.ranges.map((range, idx) => (
                      <TableRow key={idx}>
